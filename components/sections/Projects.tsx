...
export default function Projects() {
  ...
  return (
    <>
      <section id="projects" className="relative py-32 bg-slate-50 dark:bg-[#09090f]">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <span className="font-mono text-cyan-500 dark:text-cyan-400 text-sm select-none">02.</span>
            ...
            {projects.map((project) => {
              ...
              return (
                <motion.article
                  ...
                  className="group relative flex flex-col rounded-2xl bg-white/80 dark:bg-slate-900/50 border border-slate-200/70 dark:border-slate-800/70 hover:border-slate-300/80 dark:hover:border-slate-700/80 hover:bg-white dark:hover:bg-slate-900/80 transition-colors duration-300 overflow-hidden shadow-sm dark:shadow-none"
                >
                    ...
                    <div className="relative w-full h-40 overflow-hidden flex-shrink-0">
                      ...
                      {project.images!.length > 1 && (
                        <span className="absolute top-2.5 right-2.5 text-[10px] font-mono text-white/70 bg-black/50 backdrop-blur-sm rounded-md px-2 py-0.5">
                          {project.images!.length} {t("projects_screenshots")}
                        </span>
                      )}
                    </div>
                  ) : null}
                  ...
                  <div className={`flex flex-col flex-1 p-6 ${hasImages ? "pt-4" : ""}`}>
                    <div className="flex items-center justify-between mb-4">
                      <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200/70 dark:border-slate-700/50 flex items-center justify-center">
                        <FolderOpen size={16} className="text-cyan-500 dark:text-cyan-400" />
                      </div>
                    ...
                    <div className="flex-1 mb-5">
                      <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100 group-hover:text-slate-950 dark:group-hover:text-white mb-2 transition-colors">
                        {project.title}
                      </h3>
                    ...
                    <div className="border-t border-slate-200/60 dark:border-slate-800/60 pt-4">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs text-slate-400 dark:text-slate-500 hover:text-cyan-500 dark:hover:text-cyan-400 transition-colors group/btn"
                      >
